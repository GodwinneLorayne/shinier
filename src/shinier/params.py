...
class Signature(BaseModel):
    ...
    parameters: Annotated[
        Sequence[Parameter], Field(description="The parameters of the callable")
    ]
...
def convert_parameter_kind(kind: Any) -> ParameterKind:
    ...
    if hasattr(inspect.Parameter, "POSITIONAL_ONLY") and kind == getattr(
        inspect.Parameter, "POSITIONAL_ONLY"
    ):
        return ParameterKind.POSITIONAL_ONLY

    if hasattr(inspect.Parameter, "POSITIONAL_OR_KEYWORD") and kind == getattr(
        inspect.Parameter, "POSITIONAL_OR_KEYWORD"
    ):
        return ParameterKind.POSITIONAL_OR_KEYWORD

    if hasattr(inspect.Parameter, "VAR_POSITIONAL") and kind == getattr(
        inspect.Parameter, "VAR_POSITIONAL"
    ):
        return ParameterKind.VAR_POSITIONAL

    if hasattr(inspect.Parameter, "KEYWORD_ONLY") and kind == getattr(
        inspect.Parameter, "KEYWORD_ONLY"
    ):
        return ParameterKind.KEYWORD_ONLY

    if hasattr(inspect.Parameter, "VAR_KEYWORD") and kind == getattr(
...
def inspect_callable(func: Callable) -> Signature:
    ...
    if inspect_signature.return_annotation == inspect.Signature.empty:
        ...
        return_annotation = ReturnAnnotation(
            annotation=inspect_signature.return_annotation
        )
