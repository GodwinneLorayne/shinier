...
from typing import Annotated, Union, Callable, Iterator, Sequence, Iterable
...
class Flag(BaseModel):
    ...
    sentinel: Annotated[str, Field(description="Character(s) at the front which say this is a flag Eg. - or -- or /")]
    name: Annotated[str, Field(description="Character(s) which make up the name of the flag Eg. l or long")]
    full: Annotated[str, Field(description="Characters which make up the entire flag Eg. -l or --long")]

class Assignment(BaseModel):
    """A command-line flag+value, Eg. -l=5 or --length=5"""

    sentinel: Annotated[str, Field(description="Character(s) at the front which say this is an assignment Eg. =")]
    name: Annotated[str, Field(description="Character(s) which make up the name of the flag Eg. l or length")]
    flag: Annotated[str, Field(description="Character(s) which make up the entire flag Eg. -l or --length")]
    delimiter: Annotated[str, Field(description="Character(s) which separate the flag from the value Eg. =")]
    value: Annotated[str, Field(description="Character(s) which make up the value Eg. 5")]
    full: Annotated[str, Field(description="Characters which make up the entire assignment Eg. -l=5 or --length=5")]

class Value(BaseModel):
    """A command-line value, Eg. 5"""

    value: Annotated[str, Field(description="Character(s) which make up the value Eg. 5")]
    full: Annotated[str, Field(description="Characters which make up the entire value Eg. 5")]
...
def parse_single_dash_assignment(input_argument: str) -> Iterator[Argument]:
    ...
    yield Assignment(sentinel="-", name=input_argument[1], flag=input_argument[:2], delimiter="=", value=input_argument[3:], full=input_argument)
...
def parse_double_dash_assignment(input_argument: str) -> Iterator[Argument]:
    ...
    yield Assignment(sentinel="--", name=name, flag=flag, delimiter="=", value=value, full=input_argument)
...
def parse_arguments(input_arguments: InputArguments, parsers: ArgumentParsers = DEFAULT_ARGUMENT_PARSERS) -> Arguments:
